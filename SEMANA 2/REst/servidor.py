...
class RESTRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        elif self.path == "/buscar_nombres_p":
            nombresconp = [estudiante["nombre"] for estudiante in estudiantes if estudiante["nombre"].startswith("P")]
            self.send_response(200)
            self.send_header("Content-type", "application/json")
            self.end_headers()
            self.wfile.write(json.dumps(nombres_con_p).encode("utf-8"))
        elif self.path == "/ccarrera":
            carreras = {}
        ...
        elif self.path == "/testudiantes":
            testudiantes = len(estudiantes)
            response_data = {"total_estudiantes": total_estudiantes}
            self.send_response(200)
            self.send_header("Content-type", "application/json")
            self.end_headers()
